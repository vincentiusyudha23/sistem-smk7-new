<div class="w-100 bg-gray-300 rounded-lg p-5">
    <form id="form-soal-ujian" method="POST" action="{{ route('mapel.soal-ujian.store') }}">
        ...
        <p>Tanggal Ujian : {{ $sesi->tanggal_ujian->format('d-m-Y') }},  <span class="btn btn-success btn-sm text-white">Mulai : {{ $sesi->start->format('H:i') }}</span> - <span class="btn btn-error btn-sm text-white">Selesai : {{ $sesi->end->format('H:i') }}</span></p>
        <div class="w-full">
                        ...
                        @foreach ($soal_ujian as $key => $soal)
                            <li class="bg-gray-200 p-3 rounded-lg my-2 {{ $key }}">
                                <label class="form-control">
                                    ...
                                    <textarea name="soal[{{ $key }}][soal]" required class="rounded-md p-2">{{ $soal->soal }}</textarea>
                                </label>
                                    ...
                                    <div class="form-control parent-opsiSoal-{{ $loop->index + 1 }}">
                                        @foreach ($soal->opsi_soal ?? [] as $opsi)
                                            <label class="label cursor-pointer justify-start">
                                                ...
                                                <input type="text" required value="{{ $opsi }}" name="soal[{{ $key }}][opsi_soal][]" id="opsi-{{ $key }}" class="input input-bordered input-sm mx-3 w-full md:w-[50%]">
